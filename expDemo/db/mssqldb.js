...
sql.connstr = "mssql://sa:123123@./Athena";
sql.sqlserver = mssql;
...
sql.connect = function ()
{
    ...
    {
        this.isconnect = true;
        return mssql.connect(this.connstr);
    }
    return undefined;
};
/**
...
sql.queryWithParams = function (sqltext, params, func)
{
    try
    {
        ...
        if (conn)
        {
            conn.then(function ()
            {
                sql.queryWithParamsTodo(sqltext, params, func);
            });
        }
        else
        {
...
sql.queryWithParamsTodo = function (sqltext, params, func)
{
    new mssql.Request().query(sqltext).then(function (rs)
    {
        func(null, rs);
    }).catch(function (err)
    {
